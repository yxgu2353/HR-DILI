...
def load_data(data) -> 'DGLGraph Form':
    ...
    dataset = MoleculeCSVDataset(df=data,
                                 ...
                                 cache_file_path=r'result/GCN_' + str(i) + '/DGLGraph.bin',
                                 n_jobs=-1,
                                 init_mask=True)
    return dataset
...
def run_a_train_epoch(args, epoch, model, data_loader, loss_criterion, optimizer):
    ...
    Train_performance.to_csv(r'result/GCN_' + str(i) + '/Train_result.csv', mode='a', index=False)
...
def run_an_valid_epoch(model, data_loader, loss_criterion):
    ...
    Valid_performance.to_csv(r'result/GCN_' + str(i) + '/Valid_result.csv', mode='a', index=False)
...
def run_an_test_epoch(model, data_loader, loss_criterion):
    ...
    with torch.no_grad():
        ...
        Test_performance.to_csv(r'result/GCN_' + str(i) + '/Test_result.csv', mode='a', index=False)
...
def main(args, train_set, valid_set, test_set):
    ...
    stopper = EarlyStopping(patience=50, filename=r'result/GCN_' + str(i) + '/model_best.pth', metric='roc_auc_score')
    ...
    e.to_csv(r'result/GCN_' + str(i) + '/Training_loss_auc.csv')

    # Save model
    fn = r'result/GCN_' + str(i) + '/Early_stop_model.pt'
    ...
    i = 10
    with open(r'result/GCN_' + str(i) + '/test_result.txt', 'w') as f:
