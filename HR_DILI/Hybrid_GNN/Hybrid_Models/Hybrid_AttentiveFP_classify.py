...
# Loading dataset
def load_data(data, path, feat_column) -> 'DGLGraph Form':
...
def run_a_train_epoch(args, epoch, model, data_loader, loss_criterion, optimizer):
    ...
    for batch_id, batch_data in enumerate(data_loader):
        smiles, bg, features, labels, masks = batch_data
        if len(smiles) == 1:
        ...
        rdkit_tra = features.to(device)
        logits = model(bg, n_feats, e_feats, rdkit_tra)
        # print(logits)
    ...
    Train_performance.to_csv(r'result/AttentiveFP/AttentiveFP_' + str(i) + '/Train_result.csv', mode='a', index=False)
...
def run_an_valid_epoch(model, data_loader, loss_criterion):
    ...
    with torch.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            ...
            n_feats = bg.ndata.pop('hv').to(device)
            e_feats = bg.edata.pop('he').to(device)
            rdkit_val = features.to(device)
            logits = model(bg, n_feats, e_feats, rdkit_val)
    ...
    Valid_performance.to_csv(r'result/AttentiveFP/AttentiveFP_' + str(i) + '/Valid_result.csv', mode='a', index=False)
...
def run_an_test_epoch(model, data_loader, loss_criterion):
    ...
    with torch.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            ...
            n_feats = bg.ndata.pop('hv').to(device)
            e_feats = bg.edata.pop('he').to(device)
            rdkit_te = features.to(device)
            logits = model(bg, n_feats, e_feats, rdkit_te)
        ...
        Test_performance.to_csv(r'result/AttentiveFP/AttentiveFP_' + str(i) + '/Test_result.csv', mode='a', index=False)
...
def main(args, train_set, valid_set, test_set):
    i = 10
    train_loader = DataLoader(dataset=train_set, batch_size=256, shuffle=True, collate_fn=collate_graphs)
    valid_loader = DataLoader(dataset=valid_set, batch_size=256, shuffle=True, collate_fn=collate_graphs)
    test_loader = DataLoader(dataset=test_set, batch_size=256, shuffle=True, collate_fn=collate_graphs)
    ...
    # model setting
    model = AttentiveFPFeature(node_feat_size=n_feats,
    ...
    optimizer = Adam(model.parameters(), lr=0.080, weight_decay=2.312e-05)
    stopper = EarlyStopping(patience=20, filename=r'result/AttentiveFP/AttentiveFP_' + str(i) + '/model_best.pth', metric='roc_auc_score')
    ...
    e.to_csv(r'result/AttentiveFP/AttentiveFP_' + str(i) + '/Training_loss_auc.csv')

    # Save model
    fn = r'result/AttentiveFP/AttentiveFP_' + str(i) + '/Early_stop_model.pt'
    ...
    for batch_id, batch_data in enumerate(test_loader):
        smiles, bg, features, labels, masks = batch_data
        labels, masks = labels.to(device), masks.to(device)
        rdkit_eval = features.to(device)
        n_feats = bg.ndata.pop('hv').to(device)
        e_feats = bg.edata.pop('he').to(device)
        logits = torch.sigmoid(best_model(bg, n_feats, e_feats, rdkit_eval))
    ...
    i = 10
    with open(r'result/AttentiveFP/AttentiveFP_' + str(i) + '/test_result.txt', 'w') as f:
...
if __name__ == '__main__':
    ...
    train_dc = pd.read_csv('training_gnn_descriptor_' + str(i) + '.csv', low_memory=False)
    valid_dc = pd.read_csv('valid_gnn_descriptor_' + str(i) + '.csv', low_memory=False)
    test_dc = pd.read_csv('GNN_test_data_descriptor.csv', low_memory=False)

    feat_columns = train_dc.drop(['SMILES', 'label'], axis=1)
    feat_size = feat_columns.columns.tolist()
    feature_size = len(feat_size)

    train_datasets = load_data(train_dc, r'result/AttentiveFP/AttentiveFP_' + str(i) + '/train', feat_size)
    valid_datasets = load_data(valid_dc, r'result/AttentiveFP/AttentiveFP_' + str(i) + '/valid', feat_size)
    test_datasets = load_data(test_dc, r'result/AttentiveFP/AttentiveFP_' + str(i) + '/test', feat_size)
