...
def run_a_train_epoch(args, epoch, model, data_loader, loss_criterion, optimizer):
    ...
    Train_performance.to_csv(r'result/GraphSAGE_' + str(i) + '/Train_result.csv', mode='a', index=False)
...
def run_an_valid_epoch(model, data_loader, loss_criterion):
    ...
    Valid_performance.to_csv(r'result/GraphSAGE_' + str(i) + '/Valid_result.csv', mode='a', index=False)
...
def run_an_test_epoch(model, data_loader, loss_criterion):
    ...
    with torch.no_grad():
        ...
        Test_performance.to_csv(r'result/GraphSAGE_' + str(i) + '/Test_result.csv', mode='a', index=False)
...
def main(args, train_set, valid_set, test_set):
    ...
    optimizer = Adam(model.parameters(), lr=0.127, weight_decay=0.001)
    stopper = EarlyStopping(patience=50, filename=r'result/GraphSAGE_' + str(i) + '/model_best.pth', metric='roc_auc_score')
    ...
    e.to_csv('result/GraphSAGE_' + str(i) + '/Training_loss_auc.csv')

    # Save model
    fn = 'result/GraphSAGE_' + str(i) + '/Early_stop_model.pt'
    ...
    i = 10
    with open(r'result/GraphSAGE_' + str(i) + '/test_result.txt', 'w') as f:
...
if __name__ == '__main__':
    ...
    train_dc = pd.read_csv(r'Train_seed2_' + str(i) + '.csv', low_memory=False)
    valid_dc = pd.read_csv(r'Valid_seed2_' + str(i) + '.csv', low_memory=False)
    test_dc = pd.read_csv('test_data.csv', low_memory=False)

    feat_columns = train_dc.drop(['SMILES', 'label'], axis=1)
    feat_size = feat_columns.columns.tolist()
    feature_size = len(feat_size)

    train_datasets = load_data(train_dc, r'result/GraphSAGE_' + str(i) + '/train')
    valid_datasets = load_data(valid_dc, r'result/GraphSAGE_' + str(i) + '/valid')
    test_datasets = load_data(test_dc, r'result/GraphSAGE_' + str(i) + '/test')
