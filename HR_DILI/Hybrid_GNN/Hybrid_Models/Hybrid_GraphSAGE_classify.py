...
def run_a_train_epoch(args, epoch, model, data_loader, loss_criterion, optimizer):
    ...
    for batch_id, batch_data in enumerate(data_loader):
        ...
        logits = model(bg, n_feats, rdkit_tra)
        # print(logits)
    ...
    Train_performance.to_csv(r'result/GraphSAGE/GraphSAGE_' + str(i) + '/Train_result.csv', mode='a', index=False)
...
def run_an_valid_epoch(model, data_loader, loss_criterion):
    ...
    with torch.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            ...
            logits = model(bg, n_feats, rdkit_val)
    ...
    Valid_performance.to_csv(r'result/GraphSAGE/GraphSAGE_' + str(i) + '/Valid_result.csv', mode='a', index=False)
...
def run_an_test_epoch(model, data_loader, loss_criterion):
    ...
    with torch.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            ...
            logits = model(bg, n_feats, rdkit_te)
        ...
        Test_performance.to_csv(r'result/GraphSAGE/GraphSAGE_' + str(i) + '/Test_result.csv', mode='a', index=False)
...
def main(args, train_set, valid_set, test_set):
    i = 10
    train_loader = DataLoader(dataset=train_set, batch_size=128, shuffle=True, collate_fn=collate_graphs)
    valid_loader = DataLoader(dataset=valid_set, batch_size=128, shuffle=True, collate_fn=collate_graphs)
    test_loader = DataLoader(dataset=test_set, batch_size=128, shuffle=True, collate_fn=collate_graphs)
    ...
    # model setting
    model = GraphSAGEFeature(in_feats=n_feats,
    ...
    optimizer = Adam(model.parameters(), lr=0.010, weight_decay=5.861e-05)
    stopper = EarlyStopping(patience=50, filename=r'result/GraphSAGE/GraphSAGE_' + str(i) + '/model_best.pth', metric='roc_auc_score')
    ...
    e.to_csv(r'result/GraphSAGE/GraphSAGE_' + str(i) + '/Training_loss_auc.csv')

    # Save model
    fn = r'result/GraphSAGE/GraphSAGE_' + str(i) + '/Early_stop_model.pt'
    ...
    for batch_id, batch_data in enumerate(test_loader):
        ...
        n_feats = bg.ndata.pop('hv').to(device)
        logits = torch.sigmoid(best_model(bg, n_feats, rdkit_eval))
    ...
    i = 10
    with open(r'result/GraphSAGE/GraphSAGE_' + str(i) + '/test_result.txt', 'w') as f:
...
if __name__ == '__main__':
    ...
    train_datasets = load_data(train_dc, r'result/GraphSAGE/GraphSAGE_' + str(i) + '/train', feat_size)
    valid_datasets = load_data(valid_dc, r'result/GraphSAGE/GraphSAGE_' + str(i) + '/valid', feat_size)
    test_datasets = load_data(test_dc, r'result/GraphSAGE/GraphSAGE_' + str(i) + '/test', feat_size)
