...
from Descriptor_hyper import init_hyper_space
from Descriptor_utils import get_configure, mkdir_p, init_trial_path, \
...
def run_a_train_epoch(args, epoch, model, data_loader, loss_criterion, optimizer):
    ...
    for batch_id, batch_data in enumerate(data_loader):
        ...
        if len(smiles) == 1:
            ...
            continue
        labels, masks,features = labels.to(args['device']), masks.to(args['device']),features.to(args['device'])
        logits = predict(args, model, bg, features)
...
def main(args, exp_config, train_set, val_set, test_set):
    ...
    val_loader = DataLoader(dataset=val_set, batch_size=exp_config['batch_size'],shuffle=True,
                            collate_fn=collate_molgraphs)
    test_loader = DataLoader(dataset=test_set, batch_size=exp_config['batch_size'],shuffle=True,
                             collate_fn=collate_molgraphs)
...
def bayesian_optimization(args, train_set, val_set, test_set):
    ...
    trials = Trials()
    best=fmin(objective, candidate_hypers, algo=tpe.suggest,
              max_evals=args['num_evals'],trials=trials)
    Hyper_results=trials.trials
    print(len(Hyper_results))
    results.sort(key=lambda tup: tup[1])
    best_trial_path, best_val_metric = results[0]

    parameters = list(candidate_hypers.keys())
    #print(parameters)
    cols = len(parameters)
    f, axes = plt.subplots(nrows=2, ncols=4, figsize=(22, 10))

    return best_trial_path, Hyper_results, f
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ctr', '--csv-path-train', type=str, required=True,
                        help='Path to a csv file for loading a dataset')
    parser.add_argument('-cv', '--csv-path-vali', type=str, required=True,
                        help='Path to a csv file for loading a dataset')
    parser.add_argument('-cte', '--csv-path-test', type=str, required=True,
                        help='Path to a csv file for loading a dataset')
    ...
    parser.add_argument('-mo', '--model', choices=['GCN', 'GAT', 'AttentiveFP', 'GraphSAGE'],
                        default='GCN', help='Model to use (default: GCN)')
    parser.add_argument('-a', '--atom-featurizer-type',
                        choices=['canonical', 'attentivefp','weave'],
                        default='canonical',
                        help='Featurization for atoms (default: canonical)')
    parser.add_argument('-b', '--bond-featurizer-type',
                        choices=['canonical', 'attentivefp','weave'],
                        default='canonical',
    ...
    train_set = load_dataset(args, df_train)
    val_set = load_dataset(args, df_vali)
    test_set = load_dataset(args, df_test)
    ...
    if args['num_evals'] is not None:
        ...
        trial_path, Hyper_results, f = bayesian_optimization(args, train_set, val_set, test_set)
        m = np.array(Hyper_results)
        np.save('hyper.npy', m)
        # f.savefig("hyper.png", dpi=500,  bbox_inches = 'tight'
        #          )
    else:
