...
def train_results(best_model, train_x, train_y, ):
    ...
    for train_index, test_index in cv.split(train_x, train_y):
        ...
        print('Train_results ACC: {:.4f} Precision: {:.4f} Recall: {:.4f} AUC: {:.4f}  SP: {:.4f}'.format(ACC_train,
                                                                                                          Precision_train,
                                                                                                          Recall_train,
                                                                                                          AUC_train,
                                                                                                          SP_train))
    ...
    return accuracy(y_true_train, y_pred_train), precision(y_true_train, y_pred_train), recall(y_true_train, y_pred_train), auc(y_true_train, y_score_train),\
           sp(y_true_train, y_pred_train), y_true_train, y_pred_train
...
def cv_results(best_model, train_x, train_y):
    ...
    for train_index, test_index in cv.split(train_x, train_y):
        ...
        # Show this performance
        print('Valid_results ACC: {:.4f} Precision: {:.4f} Recall: {:.4f} AUC: {:.4f} SP: {:.4f}'.format(
            ACC_test,
            Precision_test,
            Recall_test,
            AUC_test,
    ...
    return accuracy(y_true_test, y_pred_test), precision(y_true_test, y_pred_test), recall(y_true_test, y_pred_test), auc(y_true_test, y_score_test),\
           sp(y_true_test, y_pred_test), y_true_test, y_pred_test
...
def test_results(best_model, test_x, test_y):
    ...
    return accuracy(y_true, y_pred), precision(y_true, y_pred), recall(y_true, y_pred), auc(y_true, y_scores), sp(y_true, y_pred), y_true, y_pred
...
def search_best_model(training_data, method_name, test_data):
    ...
    best_model = cg[0]
    best_parameter = cg[1]
    train_metrics = train_results(best_model, train_x, train_y)[:5]
    cv_metrics = cv_results(best_model, train_x, train_y)[:5]
    TV_metrics = test_results(best_model, test_x, test_y)[:5]
...
def train_main(model_list, fp_list, train_name, test_name):
    ...
    train_metrics_file = open('Hepa_train_metrics.txt', 'w')
    cv_metrics_file = open('Hepa_cv_metrics.txt', 'w')
    tv_metrics_file = open('Hepa_tv_metrics.txt', 'w')
    parameter_file = open("Hepa_parameter.txt", "w")
    train_metrics_file.write('Target\tFingerprint\tMethod\tAccuracy\tPrecision\tRecall\tAUC\tSP\n')
    cv_metrics_file.write('Target\tFingerprint\tMethod\tAccuracy\tPrecision\tRecall\tAUC\tSP\n')
    tv_metrics_file.write('Target\tFingerprint\tMethod\tAccuracy\tPrecision\tRecall\tAUC\tSP\n')
    ...
    for method_name, fp_name, train_target, test_target in model_names:
        ...
        pickle.dump(saved_model, open('Hepa_' + method_name + '_' + fp_name + '.model', 'wb'))
